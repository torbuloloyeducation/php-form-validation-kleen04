...
$name = $email = $website = $comment = $gender = $phone = $pass = $confirm_pass = "";
$submitted = false;
$count = 0; 
...
<head>
    ...
    <title>Modern PHP Form - Lab Version</title>
    <style>
        :root {
...
<div class="form-container">
    <h2>Get in Touch</h2>
    <p class="required-note">Submission attempt: <strong><?= $count ?></strong></p> <?php if ($formValid): ?>
        <div class="success-box">
            Form submitted successfully!
        </div>
    <?php endif; ?>

    <form method="post" action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>"> <input type="hidden" name="count" value="<?= $count ?>">

        <div class="field-row">
            <label for="name">Name <span style="color:var(--error-red)">*</span></label>
            <input type="text" id="name" name="name" placeholder="Jane Doe" value="<?= $name ?>"> <?php if($nameErr): ?><span class="error"><?= $nameErr ?></span><?php endif; ?>
        </div>
        ...
        <div class="field-row">
            ...
            <input type="text" id="phone" name="phone" placeholder="+63 912 1234567" value="<?= $phone ?>"> <?php if($phoneErr): ?><span class="error"><?= $phoneErr ?></span><?php endif; ?>
        </div>

        <div class="field-row">
            <label for="website">Website</label>
            <input type="text" id="website" name="website" placeholder="https://..." value="<?= $website ?>">
            <?php if($websiteErr): ?><span class="error"><?= $websiteErr ?></span><?php endif; ?> </div>

        <div class="field-row">
            <label for="pass">Password <span style="color:var(--error-red)">*</span></label>
            <input type="password" id="pass" name="pass">
        </div>

        <div class="field-row">
            <label for="confirm_pass">Confirm Password <span style="color:var(--error-red)">*</span></label>
        ...
        <div class="field-row">
            <div class="checkbox-group">
                <label class="checkbox-item">
                    <input type="checkbox" name="terms" <?= isset($_POST['terms']) ? "checked" : "" ?>> I agree to the terms and conditions <span style="color:var(--error-red)">*</span>
                </label>
            </div>
